...
export const addOrder = async (req, res) => {
    let { orderDate, toDate, address, orderCode, product, isCare, userName } = req.body;
    ...
    try {
        ...
        let neworder = await Order.create({ orderDate, toDate, address, orderCode, product, isCare: isCare || false, userName })
...
export const getAllOrderes = async (req, res) => {
    ...
    let { search, address, userName } = req.query;
    try {
        let allorders;
        let serachObject = {};
        if (search)
        ...
        if (userName)
            serachObject.userName = userName
